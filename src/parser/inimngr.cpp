...
ScavIni::ScavIni(const char *filename, const SK *t, int n)
{
   ...
   // get path where ini file is supposed to be
   if ((path=getenv("SCAV2020_PATH")) == NULL) {
      fprintf(stderr, "Warning in Scavini constructor: no environment variable for SACV2020 ==> current directory is used\n");
      inifile = (char *)filename;
   }
   else {
      s = (char *)malloc(strlen(path)+strlen(filename)+1);
      strcpy(s, path);
      strcat(s, "/");
      strcat(s, filename);
   ...
   // verify that inifile exists
   ifstream iniflow (inifile, ios::in|ios::binary);
   if (!iniflow){
      fprintf(stderr, "Error in Scavini constructor: can't open inifile\n");
      exit(EXIT_FAILURE);
   }
